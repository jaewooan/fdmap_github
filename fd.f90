...
contains
  ...
  subroutine test_diff(FDmethod)
    ...
    integer,parameter :: mg=1,pg=10,m=mg,p=pg,mb=mg-3,pb=pg+3
    integer :: i
    real :: f(mb:pb),fp(mb:pb),fm(mb:pb),x,df(m:p),dfe(mb:pb),err
    type(limits) :: L
    character(128) :: str
    ...
    do i = mb,pb
       ...
       !f(i) = (x-5.5d0)**1; dfe(i) = 1d0*(x-5.5d0)**0
       f(i) = (x-5.5d0)**2; dfe(i) = 2d0*(x-5.5d0)**1
       !f(i) = (x-5.5d0)**3; dfe(i) = 3d0*(x-5.5d0)**2
       !f(i) = (x-5.5d0)**4; dfe(i) = 4d0*(x-5.5d0)**3
       !f(i) = (x-5.5d0)**5; dfe(i) = 5d0*(x-5.5d0)**4
       !f(i) = (x-5.5d0)**6; dfe(i) = 6d0*(x-5.5d0)**5
    ...
    if (FDmethod(1:3)=='SBP') then
       print *, 'testing diff with ', FDmethod
       call diff(L,f,df)
    end if

    do i = m,p
       err = df(i)-dfe(i)
       write(str,'(i6,3e25.14)') i,df(i),dfe(i),err
       call message(str)
    end do
